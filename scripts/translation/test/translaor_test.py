...
async def main():
    ...
    questions = dataset_handler.extract_questions(filter_dataset)

    # 翻訳の実行
    translated_text = await translator.batch_translate_en2ja(questions[491:550], max_size= 10)
...
async def ajustment_speak():
    ...
    #データの読み込み
    text_list = dataset_handler.load_text_from_json("test_translated.json")

    await translator.batch_translate_text2spoken_with_filler(text_list, max_size=5)
